...
def equip(
    ...
    Args:
        hide (Optional[List[str]]): List of argument names to remove from the function's signature.
                                    This prevents AI frameworks from trying to provide them.
        inject (Optional[Dict[str, Any]]): A dictionary of arguments to forcefully inject at runtime.
                                           Values can be static, or zero-argument callables for dynamic state.
        mask_output (Optional[Callable]): A filter function to sanitize or truncate the original 
                                          function's raw return value before sending it back to the AI.
        map_inputs (Optional[Dict[str, Callable]]): A dictionary mapping AI-provided argument names to resolving functions.
                                             Used to translate faked IDs back to real internal IDs.
    ...
    def decorator(func: Callable):
        ...
        def wrapper(*args, **kwargs):
            ...
            # 2. Translation time: Resolve fake inputs to real primitives (e.g., Aliases -> Database IDs)
